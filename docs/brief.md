...
11. Одна задача одновременно детектируется D2 и D3 → в popup и агрегированном состоянии остаётся 1 (дедупликация по `taskKey`).
...
* **MVP v0.1.0**: DOM‑детекторы D1/D2, агрегатор, уведомление, popup, RU/EN (**без** бейджа, пользовательских настроек и звукового оповещения).
* **v0.2.0**: бейдж на иконке, UI/синхронизация настроек (`debounceMs`, `sound`, `autoDiscardableOff`, `showBadgeCount`), звуковое оповещение, фичефлаги детекторов.
