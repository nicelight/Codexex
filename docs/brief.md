...
* **content‑script** (на страницах `https://*.openai.com/*` и `https://*.chatgpt.com/*`):
  ...
  * Периодически/при мутациях вычисляет `activeTasks` (булев флаг + список источников), используя **детекторы**:
    ...
    * D2: кнопка **Stop/Cancel/Остановить/Отменить** в карточках задач на главной странице.
    * **D3 (v0.2.0+)**: эвристика заголовков/текста карточек. Реализована как модуль, но в v0.1.0 отключена флагом `enableCardHeuristic=false`.
...
* Использует набор селекторов `[aria-busy="true"]`, `[role="progressbar"]`, `.animate-spin`, `.loading-spinner`, `svg[role="img"] animateTransform` и собирает уникальные элементы через `collectElements()`.
* Сигналы формируются для первых пяти найденных элементов в формате ``${tagName}#index`` (см. `elementEvidence`), что позволяет понимать источник детектора.
* `count` равен количеству найденных элементов (не менее 1, если найден хотя бы один индикатор).

**D2 — Кнопка Stop/Cancel/Остановить/Отменить**

* Просматривает элементы `button`, `[role="button"]`, `a[href]`, кэширует уже распознанные узлы и сравнивает локализованный текст, `title`, `aria-label` и `data-testid` с набором ключевых слов (`stop`, `cancel`, «остановить», «отменить» и др.).
* Если рядом с кнопкой присутствуют идентификаторы `data-task-id`/`data-id`/`data-reactid`, они используются как `taskKey`.
* `count` соответствует количеству обнаруженных кнопок (без дополнительных эвристик).

**D3 — Карточки задач (эвристика, v0.2.0+)**

* Статус: реализация переносится в релиз v0.2.0+. В рамках MVP готовим только интерфейс для будущего детектора (возможность регистрации, поле `detector` в сообщениях).
* После включения необходимо учитывать элементы с data‑атрибутами `data-testid*="task"`, заголовки задач.
...
* Контент-скрипт формирует единый список сигналов, где каждой найденной сущности соответствует запись с `detector`, `evidence` и (если применимо) `taskKey`. Значение `evidence` строится через `elementEvidence` и дополняется индексом в списке.
* `ActivityScanner` объединяет результаты детекторов: `active` = логическое «ИЛИ», `count` = максимум из `result.count`, `signals` = объединённый массив без дубликатов (сравнение по `detector`, `evidence`, `taskKey`).
* `taskKey` присутствует только у сигналов, где детектор сумел идентифицировать задачу (на сегодняшний день — кнопки остановки).

**Политика решения:** активной считаем вкладку, если сработал **любой** детектор. Значение `count` отражает верхнюю границу по числу найденных сигналов и используется напрямую фоном без дополнительных коэффициентов.
