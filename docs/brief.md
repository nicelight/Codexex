...
* **content‑script** (на страницах `https://*.openai.com/*` и `https://*.chatgpt.com/*`):
  ...
  * Периодически/при мутациях вычисляет `activeTasks` (булев флаг + список источников), используя **детекторы**:
    ...
    * **D3 (v0.2.0+)**: эвристика заголовков/текста карточек (если D1/D2 недоступны).
...
* **background (service worker)**:
  ...
  * Считает `totalActiveCount` (сумма активных по всем вкладкам) для уведомлений и popup.
  * Если предыдущее значение `> 0`, новое `= 0` и выдержан антидребезг → `chrome.notifications.create(...)`.
  * Таймер `chrome.alarms` (каждую минуту) пингует вкладки, чтобы восстановить статус после сна, и сверяет время последнего heartbeat с порогом (≈45 секунд). При пропуске heartbeat инициирует повторное сканирование и помечает вкладку как «устаревшую», пока не придёт новое сообщение.
  ...
  * **v0.2.0+**: добавляет управление бейджем (`chrome.action.setBadgeText`), пользовательские настройки (`debounceMs`, `autoDiscardableOff`, `sound`, `showBadgeCount`) и звуковое уведомление.
...
> В MVP v0.1.0 используется только подмножество `MVP_SETTINGS = { debounceMs: 12000, autoDiscardableOff: true }`; поля `sound` и `showBadgeCount` вступают в силу лишь после релиза v0.2.0.
...
* В MVP (v0.1.0) параметры антидребезга и запрета авто‑выгрузки заданы константами (`debounceMs = 12000`, `autoDiscardableOff = true`); фон не управляет бейджем и звуком.
* **v0.2.0+**: настройки читаются из `chrome.storage.sync` и применяются ко всем вкладкам Codex (`true` → `chrome.tabs.update({ autoDiscardable: false })`, `false` → `chrome.tabs.update({ autoDiscardable: true })`). Новые вкладки получают актуальное значение после первого `TASKS_UPDATE` или `PING`; при включённой опции обновляется бейдж и звук уведомления.
...
* Логика снапшота опирается на `MutationObserver`, который фиксирует структурные и текстовые изменения (`characterData:true`, см. §18), чтобы обновления контента детектора D2 (и будущего D3 в v0.2.0+) попадали в обработку даже без замены узлов.
