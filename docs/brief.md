...
* **background (service worker)**:
  ...
  * В v0.1.0 хранит счётчик и решает, когда показать уведомление (без обновления бейджа; `chrome.action.setBadgeText` переносится в v0.2.0).
...
> **Статус:** настройки переносятся в релиз v0.2.0. В v0.1.0 `storage.sync` не используется, значения фиксированы в коде. Схема остаётся черновиком для следующей итерации.
...
**Политика решения:** активной считаем вкладку, если сработал **любой** детектор. Поле `count` — сумма размеров всех коллекций (для агрегации; отображение на бейдже добавим в v0.2.0). В уведомлениях показываем только факт 0/не 0.

---

## 7. UX и поведение

* **Popup (v0.1.0)**: список активных задач с названием вкладки/временной меткой; «Нет активных» — серый текст. Ссылка «Открыть Codex».
* **Иконка расширения**: в v0.1.0 остаётся без бейджа; `chrome.action.setBadgeText` и отображение числа активных задач переносим в v0.2.0.
* **Уведомление**: один раз при переходе `>0 → 0`, текст: `Все задачи в Codex завершены` + кнопка `ОК`; в v0.1.0 без звука, опциональный аудиосигнал добавим в v0.2.0 через offscreen document.
* **Настройки**: в v0.1.0 нет пользовательского UI/`storage.sync`; задержка 12 с фиксирована. Экран настроек и синхронизация вынесены в бэклог v0.2.0.
...
* Background хранит `state` с полями `tabs`, `lastTotal` и `debounce` (в `debounce.since` фиксируем момент кандидата на уведомление, в `debounce.ms` — длительность антидребезга).
* При каждом `TASKS_UPDATE` пересчитываем `totalActive` как сумму `tab.count` (общее число совпадений по вкладкам).
* Если `totalActive == 0` и раньше было `>0`, стартуем `debounce` с жёстко заданным `DEFAULT_DEBOUNCE_MS = 12000`. По истечении проверяем ещё раз; если всё ещё `0` — уведомляем.
...
* В v0.2.0 значение `debounce.ms` будет считываться из настроек (`storage.sync`), а также появится обновление бейджа с суммарным `totalActive`.
...
3. Краткий всплеск спиннера (перезапуск) → уведомление **не** показывается (антидребезг 12 с).
...
7. RU/EN локали → корректные тексты popup и уведомления.
8. Принудительно «усыпить» вкладку → дождаться `PING` и убедиться, что content‑script ставит в очередь `requestSnapshot()` и состояние восстанавливается.
...
**Запланировано к дополнению тест‑плана в v0.2.0**

* Проверка отображения бейджа с количеством активных задач.
* Тоггл настроек (например, задержка антидребезга) и их синхронизация через `storage.sync`.
* Активация звукового уведомления через offscreen document.
...
* AC‑1: При наличии хотя бы одной активной задачи в любой вкладке popup показывает список активных задач (или их количество) без рассинхрона между вкладками.
* AC‑2: Уведомление «Все задачи завершены» показывается **ровно один раз** при переходе `>0 → 0` с задержкой 12 с; повтор появляется только после новой активности.
...
* AC‑β1: Иконка расширения отображает бейдж с суммарным количеством активных задач и очищается при `totalActive = 0`.
* AC‑β2: Пользователь может менять задержку антидребезга и включать звук в UI настроек; значения синхронизируются через `storage.sync`.
* AC‑β3: При включённом звуке уведомление сопровождается аудиосигналом; при отключённом — нет.

---

## 14. План релиза

* **MVP v0.1.0**: DOM‑детекторы D1/D2, агрегатор, popup, уведомление без звука, RU/EN; задержка антидребезга фиксирована (12 с), бейджа нет.
...
// MVP v0.1.0: без бейджа и пользовательских настроек; они появятся в v0.2.0.
