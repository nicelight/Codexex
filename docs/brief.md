...
* **background (service worker)**:
  ...
  * **v0.2.0+**: включает управление бейджем (`chrome.action.setBadgeText`) и другими настройками UI.
...
* В MVP (v0.1.0) параметры антидребезга и запрета авто‑выгрузки заданы константами (`debounceMs = 12000`, `autoDiscardableOff = true`).
* **v0.2.0+**: настройки читаются из `chrome.storage.sync` и применяются ко всем вкладкам Codex (`true` → `chrome.tabs.update({ autoDiscardable: false })`, `false` → `chrome.tabs.update({ autoDiscardable: true })`). Новые вкладки получают актуальное значение после первого `TASKS_UPDATE` или `PING`.
...
12. Опция `sound` → звук присутствует/отсутствует в зависимости от настройки.
13. Включение/выключение `showBadgeCount` → бейдж появляется/очищается корректно, значения совпадают с `count`.
14. Изменение `autoDiscardableOff` и `debounceMs` через UI → настройки синхронизируются и применяются ко всем вкладкам.
