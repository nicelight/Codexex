...
## Фаза 2. Background service worker ⏳
...
- [ ] **Обработка входящих сообщений**
  ...
  - [ ] Организовать маршрутизацию сообщений: `TASKS_UPDATE`, `PING`, запрос состояния popup.
- [ ] **Агрегация состояния**
  - [ ] Реализовать `aggregator.ts` для обновления `storage.session.state` по схеме `codex.tasks/state/session.json`.
  - [ ] Поддерживать консистентность `tabs` при закрытии вкладок и рестарте service worker.
- [ ] **Уведомления и антидребезг**
  - [ ] Создать модуль `notifications.ts` для детекции перехода `totalActiveCount` от >0 к 0 с учётом `debounceMs`.
  - [ ] Инкапсулировать логику одиночного уведомления и очистку уведомлений при новых задачах.
- [ ] **Алгоритм будильников**
  - [ ] Реализовать `alarms.ts` с расписанием `chrome.alarms` и обработчиком повторных `PING` во вкладки.
  - [ ] Добавить применение `autoDiscardableOff` ко вкладкам Codex после обновления состояния.

## Фаза 3. Content-script ⏳
...
- [ ] **Агрегация сигналов**
  - [ ] Объединить сигналы, вычислить `count`, формировать DTO `TASKS_UPDATE` согласно контракту.
  - [ ] Настроить throttle/anti-bounce отправки (используя `requestIdleCallback`/таймеры).
- [ ] **Взаимодействие с background**
  - [ ] Обрабатывать входящие `PING`/`RESET` (если потребуется) сообщения.
  - [ ] Интегрировать fail-safe таймер на случай отсутствия мутаций (периодическая проверка DOM).
